...
for rg in "${resource_groups[@]}"; do
    ...
    az network nsg list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Key Vaults:"
    az keyvault list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Managed Identities:"
    az identity list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Network Security Groups:"
    az network nsg list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Private Endpoints:"
    az network private-endpoint list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Network Interfaces:"
    az network nic list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Route Tables:"
    az network route-table list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "SQL Databases:"
    az sql db list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "SQL Servers:"
    az sql server list --resource-group "$rg" --query "[].{Name:name}" -o table

    echo "Storage Accounts:"
    az storage account list --resource-group "$rg" --query "[].{Name:name}" -o table
